...
* @type : service
* @author : linfeng
* @created : Fri May 11 2018 13:35:16 GMT+0800 (中国标准时间)
...
function server.start(conf)
	...
	local CMD = {
		...
		clean = assert(conf.clean_handler),
	...
	function handler.disconnect(fd)
		...
		if c then
			...
			if conf.disconnect_handler then
				conf.disconnect_handler(fd, c.username)
			end
		end
	end
	...
	function handler.warning( fd, size )
		LOG_WARNING("too much data(%d) wait for send on fd(%d), close socket", size, fd)
	...
	local function do_auth(fd, message, addr)
		...
		local u = user_online[username]
		if u == nil then
			local rid, subId, gameNode = server.userid(username)
			rid = tonumber(rid)
			-- try call gameNode for roleInfo
			Common.rpcCall( gameNode, "ChatProxy", "forceLoginChatServer", rid, subId )
		...
		if idx <= u.version or not u.secret then
	...
	local function do_request(fd, message)
		...
		if p == nil then
			...
			elseif result:len() > 0 then
				...
				if result:len() > Enum.CompressMinSize then
					local zlibCompress = zlib.deflate()
					-- compress
					result = zlibCompress(result, "finish")
					compressFlag = 1
				end
				p[2] = string.pack(">s2", result .. string.pack(">I4", session) .. string.pack(">B", compressFlag) )
				]]
				p[2] = string.pack(">s2", result .. string.pack(">I4", session) .. string.pack(">B", 0) )
			end
		...
		if connection[fd] and p[2] then
			...
			--[[
			local sendDatas, allPackSize = Common.SplitPackage( p[2] )
			for msgIndex, msg in pairs(sendDatas) do
