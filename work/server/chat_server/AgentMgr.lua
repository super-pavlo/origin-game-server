...
function response.addRid( _rid, _username, _handle )
    ...
    LOG_INFO("AgentMgr addRid(%d) ref(%d)", _rid, agentRidHandles[_rid].ref)
end

function response.delRid( _rid )
    if agentRidHandles[_rid] == nil then return end
    agentRidHandles[_rid].ref = agentRidHandles[_rid].ref - 1
    LOG_INFO("AgentMgr delRid(%d) ref(%d)", _rid, agentRidHandles[_rid].ref)
