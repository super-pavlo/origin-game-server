...
{	
	...
	{
		printf("decodeTileIdTile fail, idx(%d) maxTiles(%d)\n", idx, (unsigned int)m_params.maxTiles);
		return DT_FAILURE | DT_INVALID_PARAM;
	}
	...
	if (tile->salt != salt)
	{
		printf("decodeTileIdSalt fail, tile->salt(%d) salt(%d)\n", tile->salt, salt);
		return DT_FAILURE | DT_INVALID_PARAM;
	}
	...
	if (dtStatusFailed(status))
	{
		printf("dtDecompressTileCacheLayer fail\n");
		return status;
	}
	...
	if (dtStatusFailed(status))
	{
		printf("dtBuildTileCacheRegions fail\n");
		return status;
	}
	...
	if (!bc.lcset)
	{
		printf("dtAllocTileCacheContourSet fail\n");
		return DT_FAILURE | DT_OUT_OF_MEMORY;
	}
	...
	if (dtStatusFailed(status))
	{
		printf("dtBuildTileCacheContours fail\n");
		return status;
	}
	...
	if (!bc.lmesh)
	{
		printf("dtAllocTileCachePolyMesh fail\n");
		return DT_FAILURE | DT_OUT_OF_MEMORY;
	}
	status = dtBuildTileCachePolyMesh(m_talloc, *bc.lcset, *bc.lmesh);
	if (dtStatusFailed(status))
	{
		printf("dtBuildTileCachePolyMesh fail\n");
		return status;
	}
	...
	{
		printf("dtCreateNavMeshData fail\n");
		return DT_FAILURE;
	}
